...
import { create, index } from "@/actions/admin/categories";
...
export default function Create() {
  ...
  return (
    <form action={formAction}>
      ...
      <div className="mb-6">
        <Input label="نام انگلیسی" name="slug" errorText={state?.errors?.slug} />
      </div>
      ...
      <div className="mb-6">
        <select name="parent" id="parent" className="*:text-black">
          ...
          {categories.map((category) => (
            <option value={category?.id} key={category?.id}>{category?.name} - {category?.slug}</option>
          ))}
        </select>
      </div>
