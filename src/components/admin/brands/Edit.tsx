...
import { ShowBrandInterface } from "@/contracts/admin/brands";
import { show, update } from "@/actions/admin/brands";

export default function Edit({ id }: { id: number }) {
  const router = useRouter();
  const [brand, setBrand] = useState<ShowBrandInterface>();
    ...
    if (state.success || Object.keys(state.errors).length > 0) {
      if (state.success) {
        Swal.fire({
          ...
          router.push("/admin-panel/management/brands");
        });
      }
    }
  ...
  return (
    <form action={formAction}>
      <div className="mb-6">
        <Input label="نام" name="name" defaultValue={brand?.name} errorText={state?.errors?.name} />
      </div>

      <div className="mb-6">
        <Input label="نام انگلیسی" name="slug" defaultValue={brand?.slug} errorText={state?.errors?.slug} />
      </div>
      ...
      <div className="flex items-center gap-3">
        ...
        <Link
          href="/admin-panel/management/brands"
